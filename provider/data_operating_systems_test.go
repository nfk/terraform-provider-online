...
func TestDataOperatingSystem(t *testing.T) {
	onlineClientMock.On("GetOperatingSystems", 123).Return(&online.OperatingSystems{online.OperatingSystem{
		OS: online.OS{
			Name:    "centos",
			Version: "CentOS 7.6",
		},
		...
		Steps: []resource.TestStep{
			...
			`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("data.online_operating_system.test", "id", "305"),
				),
			},
			{
				ImportStateVerify: false,
